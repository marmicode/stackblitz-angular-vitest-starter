...
import {
  readFile as readFile$1,
  mkdir,
  writeFile as writeFile$1,
...
import {
  resolve as resolve$1,
  basename as basename$1,
  dirname as dirname$1,
} from "node:path";
...
function readTag(text, pos, startBracket, context) {
  ...
  const descriptor = escapedDescriptor
    ...
    : (_text_slice_match = text
        .slice(pos)
        .match(startBracket === "{" ? /^\w+|^[^}>/]/ : /^\w+/)) === null ||
      _text_slice_match === undefined
    ? undefined
    : _text_slice_match[0];
  assertDescriptor(descriptor, text, pos);
  pos += descriptor.length;
  ...
  const repeatModifier =
    (_text_slice_match_ =
      (_text_slice_match1 = text.slice(pos).match(/^>\d+/)) === null ||
      _text_slice_match1 === undefined
        ? undefined
        : _text_slice_match1[0]) !== null && _text_slice_match_ !== undefined
      ? _text_slice_match_
      : "";
  pos += repeatModifier.length;
  const releaseSelfModifier =
    text[pos] === "/" || (!repeatModifier && text[pos] === ">")
      ? text[pos]
      : "";
...
function getErrorMessage(expected, found, text, context) {
  return `Expected ${expected} but found "${
    found !== null && found !== undefined ? found : ""
  }" in "${text}"
    ...
    const keyDef =
      (_keyboardMap_find = keyboardMap.find((def) => {
        if (type === "[") {
          var _def_code;
          return (
            ((_def_code = def.code) === null || _def_code === undefined
              ? undefined
              : _def_code.toLowerCase()) === descriptor.toLowerCase()
          );
        } else if (type === "{") {
          var _def_key;
          return (
            ((_def_key = def.key) === null || _def_key === undefined
              ? undefined
              : _def_key.toLowerCase()) === descriptor.toLowerCase()
          );
        }
        return def.key === descriptor;
      })) !== null && _keyboardMap_find !== undefined
        ? _keyboardMap_find
        : {
            key: "Unknown",
            code: "Unknown",
...
async function getBrowserProvider(options, project) {
  ...
  if (typeof options.provider.providerFactory !== "function") {
    throw new TypeError(
      `The "${name}" browser provider does not provide a "providerFactory" function. Received ${typeof options
...
async function resolveOrchestrator(globalServer, url, res) {
  let sessionId = url.searchParams.get("sessionId") ?? [...globalServer.vitest._browserSessions.sessionIds][0];
  // it's possible to open the page without a context
  if (!sessionId) {
    const contexts = [...globalServer.children].flatMap((p) => [
      ...p.state.orchestrators.keys(),
    ]);
  ...
  const browserProject =
    session?.project.browser || [...globalServer.children][0];
  if (!browserProject) {
    return;
  }
  ...
  if (
    sessionId &&
    sessionId !== "none" &&
    !globalServer.vitest._browserSessions.sessionIds.has(sessionId)
  ) {
    return;
  }
  const injectorJs =
  ...
  const injector = replacer(injectorJs, {
    __VITEST_PROVIDER__: JSON.stringify(
      browserProject.config.browser.provider?.name || "preview"
    ),
  ...
  if (!globalServer.orchestratorScripts) {
    globalServer.orchestratorScripts = (
      await globalServer.formatScripts(
  ...
  if (globalServer.config.browser.ui) {
    const manifestContent =
      globalServer.manifest instanceof Promise
...
async function resolveTester(globalServer, url, res, next) {
  ...
  const project = globalServer.vitest.getProjectByName(
...
function createTesterMiddleware(browserServer) {
  return async function vitestTesterMiddleware(req, res, next) {
    ...
    const url = new URL(req.url, "http://localhost");
    if (
      url.pathname !== browserServer.prefixTesterUrl ||
...
async function getUserEventImport(provider, resolve) {
  ...
  if (!resolved) {
    throw new Error(
      `Failed to resolve user-event package from ${previewDistRoot}`
    );
  }
  return `\
import { userEvent as __vitest_user_event__ } from '${slash$1(
    `/@fs/${resolved.id}`
  )}'
  ...
  return [
    {
      ...
      async configureServer(server) {
        ...
        if (uiEnabled) {
          // eslint-disable-next-line prefer-arrow-callback
          server.middlewares.use(
            `${base}__screenshot-error`,
            function vitestBrowserScreenshotError(req, res) {
              if (!req.url) {
                res.statusCode = 404;
                res.end();
                return;
        ...
        server.middlewares.use((req, res, next) => {
          ...
          if (
            !isValidApiRequest(parentServer.config, req) ||
            !contentType ||
            !path
          ) {
            return next();
    ...
    },
    {
      ...
      async config() {
        ...
        if (parentServer.vitest.coverageProvider) {
          ...
          if (provider === "v8") {
            const path = tryResolve("@vitest/coverage-v8", [
              parentServer.config.root,
            ]);
            if (path) {
              entries.push(path);
            ...
            const path = tryResolve("@vitest/coverage-istanbul", [
              parentServer.config.root,
            ]);
            if (path) {
              entries.push(path);
        ...
        const provider =
          parentServer.config.browser.provider ||
          [...parentServer.children][0]?.provider;
        ...
        const fileRoot = browserTestFiles[0]
          ? dirname(browserTestFiles[0])
          : project.config.root;
    ...
    },
    {
      ...
      transform(code, id) {
        if (
          id.includes(parentServer.vite.config.cacheDir) &&
          id.includes("loupe.js")
        ) {
    ...
    },
    {
      ...
      transform: {
        ...
        handler(code, id) {
          ...
          s.prepend(
            `Object.defineProperty(import.meta, 'vitest', { get() { return typeof __vitest_worker__ !== 'undefined' && __vitest_worker__.filepath === "${filename.replace(
              /"/g,
              '\\"'
            )}" ? __vitest_index__ : undefined } });\n`
          );
          return {
            code: s.toString(),
      ...
      config() {
        ...
        const esbuildPlugin = {
          ...
          setup(build) {
            ...
            build.onResolve(
              { filter: /^@vue\/(test-utils|compiler-core)$/ },
              (args) => {
...
var __classPrivateFieldGet =
  (null && null.__classPrivateFieldGet) ||
...
const screenshot = async (context, name, options = {}) => {
  ...
  const { buffer, path } = await context.triggerCommand(
...
const pixelmatch = (reference, actual, { createDiff, ...options }) => {
  ...
  let allowedMismatchedPixels = Math.min(
    ...
    (optionsWithDefaults.allowedMismatchedPixelRatio ??
      Number.POSITIVE_INFINITY) * imageArea
  );
...
function getComparator(comparator, context) {
  ...
  const customComparators =
    context.project.config.browser.expect?.toMatchScreenshot?.comparators;
...
function resolveOptions({ context, name, options, testName }) {
  ...
  const resolvePathData = {
    ...
    screenshotDirectory: relative(
      ...
      join(
        root,
        context.project.config.browser.screenshotDirectory ?? "__screenshots__"
      )
    ),
...
const screenshotMatcher = async (context, name, testName, options) => {
  ...
  const reference =
    referenceFile &&
    (await codec.decode(await readFile$1(paths.reference), {}));
  ...
  const value = await (timeout === 0
    ? stableScreenshot
    : Promise.race([
        stableScreenshot,
        asyncTimeout(timeout).finally(() => {
  ...
  if (reference === null || updateSnapshot === "all") {
    ...
    const referencePath = shouldCreateReference
      ? paths.reference
...
async function getStableScreenshots({
  ...
  while (signal.aborted === false) {
    ...
    const comparatorResult = (
      await comparator(image1, image2, {
        ...comparatorOptions,
        createDiff: false,
      })
    ).pass;
...
class ProjectBrowser {
  ...
  constructor(project, base) {
    ...
    this.testerHtml = readFile$1(testerHtmlPath, "utf8").then(
      (html) => (this.testerHtml = html)
    );
  }
  get vite() {
...
function wrapConfig(config) {
  return {
    ...
    testNamePattern: config.testNamePattern
      ? config.testNamePattern.toString()
      : undefined,
  };
}
  ...
  async formatScripts(scripts) {
    ...
    const promises = scripts.map(
      async ({ content, src, async, id, type = "module" }, index) => {
        const srcLink =
          (src
            ? (await server.pluginContainer.resolveId(src))?.id
            : undefined) || src;
        const transformId =
          srcLink ||
        ...
        const contentProcessed =
          content && type === "module"
            ? (await server.pluginContainer.transform(content, transformId))
                .code
            : content;
        return {
          tag: "script",
  ...
  async function onMessage(data, ...extra) {
    ...
    if (msg.t === TYPE_REQUEST) {
      ...
      const fn = await (resolver
        ? resolver(method, functions[method])
        : functions[method]);
      if (!fn) {
      ...
      if (msg.i) {
        ...
        if (!error) {
          try {
            await post(
              serialize({ t: TYPE_RESPONSE, i: msg.i, r: result }),
              ...extra
            );
            return;
          } catch (e) {
        ...
        try {
          await post(
            serialize({ t: TYPE_RESPONSE, i: msg.i, e: error }),
            ...extra
          );
        } catch (e) {
          if (options.onGeneralError?.(e, method, args) !== true) throw e;
    ...
    const rpc = createBirpc(
      {
        ...
        async registerMock(sessionId, module) {
          ...
          if (module.type === "manual") {
            const manualModule = ManualMockedModule.fromJSON(
              module,
              async () => {
                const { keys } = await rpc.resolveManualMock(module.url);
                return Object.fromEntries(keys.map((key) => [key, null]));
...
const createBrowserServer = async (options) => {
  ...
  if (project.vitest.version !== version) {
    project.vitest.logger.warn(
      c.yellow(
        `Loaded ${c.inverse(
          c.yellow(` vitest@${project.vitest.version} `)
        ...
        if (msg.includes("optimized dependencies changed. reloading")) {
          logger.warn(
            [
              ...
              c.yellow(
                `For a stable experience, please add mentioned dependencies to your config\'s ${c.bold(
